...
struct doca_fib_tbl *doca_create_fib_tbl(uint32_t _size)
{
	...
	uint32_t size = (uint32_t)(!rte_is_power_of_2(_size))
			    ? rte_align32pow2(_size)
			    : _size;
	...
	table = (struct doca_fib_tbl *)malloc(
	    sizeof(struct doca_fib_tbl) +
	    sizeof(struct doca_fib_tbl_entry) * size);
	if (table == NULL) {
