...
static void doca_id_pool_init(struct doca_id_pool *pool, struct doca_id_pool_cfg *cfg)
{
    int i;
    int n = cfg->min;
...
struct doca_id_pool *doca_id_pool_create(struct doca_id_pool_cfg *cfg)
{
	...
	pool = (struct doca_id_pool *) malloc(sizeof(struct doca_id_pool)+sizeof(int)*cfg->size);
	if(pool != NULL){
